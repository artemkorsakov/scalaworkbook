...
| Тип           | Описание                                                                                                                                                                          |
|---------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `val`           | Создает неизменяемую переменную, подобную `final` в Java. Желательно всегда создавать переменную с помощью `val`, если только нет причины, по которой нужна изменяемая переменная. |
| `var`           | Создает изменяемую переменную. Желательно использовать только в редких случаях, когда содержимое переменной будет меняться с течением времени.                              |
...
Он определяет определенные универсальные методы, такие как `equals`, `hashCode` и `toString`.
...
В настоящее время использование `null` считается плохой практикой. 
