...
После прогона `list` представляет `Vector`, содержащий отображаемые значения. Вот как работает это выражение:
- Выражение `for` начинает перебирать значения в диапазоне `(10, 11, 12)`. Сначала он работает со значением `10`, 
умножает его на `2`, затем выдает этот результат - `20`.
- Далее он работает с `11` — вторым значением в диапазоне. Он умножает его на `2`, 
