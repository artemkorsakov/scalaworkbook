...
Импорт по типу предоставляет более конкретную альтернативу импорту с подстановочными знаками, 
что делает более понятным, что импортируется:
...
Импорт всех `given` экземпляров параметризованного типа выражается аргументами с подстановочными знаками. 
Например, когда есть такой объект:
...
Этот оператор импорта импортирует экземпляры `intOrd`, `listOrd` и `ec`, но пропускает экземпляр `im`, 
...
Затем этот метод можно вызывать:
