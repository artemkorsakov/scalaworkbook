...
Совершенно новая функция [контекстных функций](@DOCS_LINK@type-system/types-dependent-function) 
...
- существует отдельный способ [импорта givens](@DOCS_LINK@abstractions/ca-given-imports), 
который не позволяет им прятаться в море обычного импорта
- существует единственный способ определить [неявное преобразование](@DOCS_LINK@abstractions/ca-implicit-conversions), 
