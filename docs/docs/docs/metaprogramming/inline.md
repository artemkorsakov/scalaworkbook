...
Это означает, что `logging` рассматривается как _постоянное значение_, эквивалентное его правой части `false`. 
...
Правила следующие:
1. Если встроенный метод `f` реализует или переопределяет другой, не встроенный метод, 
...
2. Встроенные методы фактически являются final.

3. Встроенные методы также могут быть абстрактными. 
